...
from tracker import YOLODetector, ByteTracker, Annotators, filter_on_terrain
...
def main():
    ...
    out = cv2.VideoWriter(str(VIDEO_OUT), cv2.VideoWriter_fourcc(*"mp4v"), 30.0, (w, h))
    ...
    while True:
        ...
        labels    = [f"#{tid}" for tid in dets.tracker_id]
        ...
        for i, tid in enumerate(dets.tracker_id):
            ...
            rec.record(frame_id, int(tid), x, y, is_att, is_def)
            txt = "Attaque" if is_att else "Defense"
            col = (0,255,0) if is_att else (0,0,255)
            cv2.putText(annotated, txt, (int(x),int(y)-10), cv2.FONT_HERSHEY_SIMPLEX,0.5, col,2)
