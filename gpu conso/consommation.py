...
# Initialiser NVML pour monitorer l'utilisation GPU
pynvml.nvmlInit()
handle = pynvml.nvmlDeviceGetHandleByIndex(0)

def log_gpu_usage():
    """
    Affiche l'utilisation de la mémoire GPU et le taux d'utilisation du GPU.
    """
    mem_info = pynvml.nvmlDeviceGetMemoryInfo(handle)
    util = pynvml.nvmlDeviceGetUtilizationRates(handle)
    total = mem_info.total / 1024**2  # Convertir en Mo
    used = mem_info.used / 1024**2
    free = mem_info.free / 1024**2
    print(f"GPU Memory Total: {total:.2f} Mo, Utilisé: {used:.2f} Mo, Libre: {free:.2f} Mo, Utilisation GPU: {util.gpu}%")
...
def main(opt):
    ...
    while True:
        ...
        # --- Affichage de l'utilisation du GPU ---
        log_gpu_usage()
